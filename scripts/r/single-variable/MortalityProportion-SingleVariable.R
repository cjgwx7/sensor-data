...
df2 <- df %>%
     dplyr::select(-c(WaterMedicationType, Comments, Days, Inventory, MortalityProportion)) # nolint

df3 <- df %>%
    dplyr::select(Days, Inventory, MortalityProportion)
...
write.csv(df_out,
          file = "/group/deckerlab/cjgwx7/sensor-data/results/MortalityProportion-SingleVariable.csv", # nolint
          sep = ",",
          row.names = FALSE,
          col.names = TRUE)