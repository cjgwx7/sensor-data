...
df2 <- df %>%
    dplyr::select(-c(GlobalID:Turn, CalendarDate:NumericDate,
                     Inventory, WaterMedicationType, Comments, ReHS)) # nolint
...
for (i in c(4:ncol(df2))) {
    ...
    null_fit <- lmer(ReHS ~ (1 + Days + I(Days^2) + I(Days^3) | Site_Room_Turn), data = fit_df, REML = FALSE) # nolint
...
write.csv(df_out,
          file = "/group/deckerlab/cjgwx7/sensor-data/results/single-variable/ReHS-SingleVariable.csv", # nolint
          sep = ",",
          row.names = FALSE,
          col.names = TRUE)