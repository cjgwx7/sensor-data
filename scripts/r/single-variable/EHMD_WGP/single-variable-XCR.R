...
df <- df %>%
    rename(DepVar = EHMD_WGP_Lag0,
           Covariate = Days_Lag0) %>%
    select(-contains("Inventory"), -contains("Marketing"),
           -contains("Euth"), -contains("Dead"),
           -contains("TotalMortality"), -contains("Days"), -contains("GP")) %>%
    rename(EHMD_WGP_Lag0 = DepVar,
           Days_Lag0 = Covariate) %>%
    select(Order:GrowthPeriod, Days_Lag0, EHMD_WGP_Lag0,
           Penicillin_Lag0:Color_ReHS_Red_Lag21)

cat("Fitting regression models...\n")
fit_df <- single_variable_binom(df = df, y = "EHMD_WGP_Lag0", x_pos = c(17:ncol(df)), covariate = "Days_Lag0", int = FALSE, random_effect = "Site_Turn")
