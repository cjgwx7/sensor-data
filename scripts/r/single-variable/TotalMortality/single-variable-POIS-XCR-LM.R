...
# Date: 8/25/2022                                                             #
# Purpose: single variable poisson regression for total mortality             #
#                                                                             #
###############################################################################
...
source("/group/deckerlab/cjgwx7/sensor-data/scripts/r/source/single_variable_pois.R")
...
df <- df %>%
    filter(Days_Lag0 >= 84 & Days_Lag0 < 126)

cat("Fitting regression models...\n")
fit_df <- single_variable_pois(df = df, y = "TotalMortality_Lag0", x_pos = c(14:22, 27, 29:1399), covariate = "Days_Lag0", int = FALSE, random_effect = "Site_Turn")
