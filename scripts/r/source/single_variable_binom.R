single_variable_binom <- function(df, y, x_pos, covariate, int, random_effect) { # nolint
  ...
  if (length(covariate) == 1 & int == TRUE & length(random_effect) == 1) {
    ...
    for (i in seq_len(ncol(X_df))) {
      ...
      fit <- glmer(Y ~ X * cov + (1 | re), family = binomial())
    ...
    for (i in seq_len(ncol(X_df))) {
      ...
      fit <- glm(Y ~ X * cov, family = binomial())
      fit_summary <- summary(fit)
      fit_coef <- coef(fit_summary)
    ...
    for (i in seq_len(ncol(X_df))) {
      ...
      fit <- glmer(Y ~ X + (1 | re), family = binomial())
      fit_summary <- summary(fit)
      fit_coef <- coef(fit_summary)
      variance <- VarCorr(fit)
