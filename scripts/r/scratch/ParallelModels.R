...
indvars <- df %>%
    dplyr::select(-c(Order:Inventory, WaterMedicationType, Comments, TotalMortality)) # nolint

var_names <- colnames(indvars)

fit <- function(x) {

    results <- coef(summary(glmer(depvars ~ x + (1 + Days + I(Days^2) | Site_Room_Turn), family = poisson(), data = df))) # nolint

}

fit_results <- mclapply(indvars, fit, mc.cores = 50)
...
write.csv(fit_results,
          file = "TotalMortality-SingleVariable.csv",
          sep = ",",
          row.names = FALSE,
          col.names = TRUE)
