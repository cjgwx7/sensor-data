...
### FOR loop 5: Days and Cubic Inventory variable(s) included
df_list <- list()
...
for (i in lags) {
    ...
    for (j in c(2:ncol(df_fit))) {
        ...
        inv <- df_fit %>%
            dplyr::select(contains("StdInventory")) %>%
            dplyr::pull(.)
        fit <- glm(y ~ x + days + inv + I(inv^2) + I(inv^3), family = binomial())
        summary_fit <- summary(fit)
        coef_fit <- coef(summary_fit)
        summary_fit_df <- tibble(`Dependent Variable` = y_variable,
                                 `Independent Variable` = x_variable,
...
write.csv(df_out,
          file = "/group/deckerlab/cjgwx7/sensor-data/results/single-variable/HMD_Global-SingleVariable.csv",
          sep = ",",
          row.names = FALSE,
          col.names = TRUE)