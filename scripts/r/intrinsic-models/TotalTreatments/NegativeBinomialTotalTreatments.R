...
for (i in seq_len(length(model_list))) {
    ...
    if (grepl("glmer", model_name) == FALSE) {
        ...
        start <- Sys.time()
        model_fit <- glm.nb(model_formula,
                            data = df)
        aic <- AIC(model_fit)
        end <- Sys.time()
        ...
        start <- Sys.time()
        model_fit <- glmer.nb(model_formula,
                              data = df)
        aic <- AIC(model_fit)
        end <- Sys.time()
...
path_out <- paste("/group/deckerlab/cjgwx7/sensor-data/results/intrinsic-models/", variable, "/NegativeBinomial.csv", sep = "") # nolint
